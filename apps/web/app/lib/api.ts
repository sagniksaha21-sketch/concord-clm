...
export const saveAgreementDraft = (id: string, body: { confirmTrackedChanges?: boolean; prepareExternalCopy?: boolean; templateId?: string; sections: import('@concord/shared').DraftSection[]; revision: number; reason?: string; sourceDocumentId?: string }) => portalJson<import('@concord/shared').AgreementWorkspace>(`/agreements/${encodeURIComponent(id)}/draft`, jsonBody('POST', body));
...
export interface ApprovalWorkspaceData { requirements?: import('@concord/shared').ApprovalRequirement[]; steps: { id: string; approverEmail: string; approverName: string; reason: string; decision: string; comment?: string; decidedAt?: string }[]; available: { id: string; name: string; email: string }[]; recommendation: string; decision?: string; documentId?: string; documentSha256?: string; contractVersion?: string; canDecide: boolean; outlookConfigured: boolean; }
...
export interface AgreementEditorSource { preserveFormatting?: boolean; lockedSections?: string[]; documentId: string | null; revision: number; filename?: string; sections: import('@concord/shared').DraftSection[]; original: import('@concord/shared').DraftSection[]; trackedChanges: boolean; notice: string; }
...
export interface PolicySettings { policies: import('@concord/shared').ApprovalPolicy[]; approvers: { id: string; name: string; email: string }[]; }
export const getApprovalPolicies = () => portalJson<PolicySettings>('/approval-policies');
export const saveApprovalPolicy = (policy: import('@concord/shared').ApprovalPolicy) => portalJson<PolicySettings>('/approval-policies',jsonBody('POST',policy));
export interface GuestAccessRecord { id: string; name: string; email: string; organisation: string; contractId: string; contract: { title: string; executedAt?: string }; expiresAt: string; revokedAt?: string; allowDownload: boolean; allowRedline: boolean; allowUpload: boolean; version: number; executedArchiveId?: string; }
export const getGuestAccess = () => portalJson<GuestAccessRecord[]>('/guest-access');
export interface CommitmentExtractionStatus { status: string; detail?: string; model?: string; source?: string; }
export const commitmentExtraction = (id: string, retry = false) => portalJson<CommitmentExtractionStatus>(`/agreements/${encodeURIComponent(id)}/commitment-extraction`, retry ? jsonBody('POST',{}) : undefined);
