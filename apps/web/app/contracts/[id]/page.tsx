...
const currentPanel = (stage: string) => ({ intake: 'request', drafting: 'draft', review: 'review', approval: 'approval', signature: 'signature', active: 'overview', renewal: 'obligations' } as Record<string,string>)[stage] ?? 'overview';
export default function AgreementPage({ params }: { params: { id: string } }) {
  ...
  const tabs = executed ? [['overview','Overview'],['executed','Executed agreement'],['obligations','Obligations & key dates'],['versions','Versions'], ...(data.request ? [['input','Business input']] : []), ...(data.permissions.includes('audit:read') ? [['activity','Activity & audit']] : [])] : [['overview','Overview'], ...(data.request ? [['request','Request']] : []), ...(canWrite ? [['draft','Draft']] : []), ['review','Review'], ...(['review','approval','signature'].includes(c.stage) ? [['approval','Approval']] : []), ...(canSign && c.stage === 'signature' ? [['signature','Signature']] : []), ...(data.request && c.stage !== 'intake' ? [['input','Business input']] : []), ['versions','Versions'], ...(data.permissions.includes('audit:read') ? [['activity','Activity']] : [])];
  ...
  <section className="agreement-focus card"><div><span className="section-kicker">{executed ? 'Permanent agreement record' : c.waitingOnClient ? 'Waiting for business input' : 'Next action'}</span><h3>{executed ? 'Signed, preserved and ready to manage.' : c.nextAction}</h3><p>{executed ? `Executed ${requestDate(data.archive!.completedAt)}` : c.waitingOnClient ? `${c.businessOwner ?? 'Requestor'} owns the next response.` : `${c.ownerName ?? 'Legal team'} owns the next step.`}</p></div><dl><div><dt>Legal owner</dt><dd>{c.ownerName ?? 'Not assigned'}</dd></div><div><dt>{executed ? 'Executed' : 'Target date'}</dt><dd>{executed ? requestDate(data.archive!.completedAt) : c.dueDate ? requestDate(c.dueDate) : 'Not set'}</dd></div></dl></section>
  ...
  {selected === 'review' && <><AgreementReview id={c.id} />{canRoute && c.stage === 'review' && <div className="sticky-context req-actions"><button className="btn" onClick={() => setPanel('draft')}>Edit Agreement</button><button className="btn btn-gold" onClick={() => setPanel('approval')}>Continue to approval →</button></div>}</>}
