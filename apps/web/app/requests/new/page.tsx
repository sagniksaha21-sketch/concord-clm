      ...
      <section className="req-panel"><ol className="req-steps" aria-label="Request progress">{['The agreement', 'Commercial terms', 'Your legal contact'].map((label, index) => <li key={label} aria-current={step === index ? 'step' : undefined}><span>{index + 1}</span>{label}</li>)}</ol>
        <form ref={formRef} className="req-form" onSubmit={submit} aria-busy={busy}><fieldset disabled={busy}><div key={step} className="req-step-view"><h3 ref={headingRef} tabIndex={-1}>{['Start with the essentials', 'Set out the terms', 'Choose who you work with'][step]}</h3><p className="req-help">{['Your request will be raised as ' + options.requester.name + '.', 'Add what you know. Your lawyer can advise on details that are still being agreed.', 'Choose a legal team member from your organisation. They will receive this request in Concord.'][step]}</p>
          ...
          {step === 1 && <div className="req-fields">
            ...
            <label className="req-field">Proposed start date<input type="date" value={data.terms.startDate ?? ''} onChange={e => term('startDate', e.target.value)} /></label>
            <label className="req-field">Proposed end date<input type="date" min={data.terms.startDate} value={data.terms.endDate ?? ''} onChange={e => term('endDate', e.target.value)} /></label>
          ...
          {step === 2 && <><div className="req-fields">
            ...
            <label className="req-field">When do you need Legal’s first draft or review? *<input type="date" required value={data.requestedByDate} onChange={e => field('requestedByDate', e.target.value)} /><small>This is your requested date; Legal will confirm timing.</small></label>
