...
export default function RequestsPage() {
  ...
  return <div className="req-page">
    <RequestHeader title="Legal Contract Requests" description="Share your terms with Legal, choose who you want to work with and follow each request from one place." action={<Link className="btn btn-gold" href="/requests/new"><IconPlus />Request a Contract</Link>} />
    <a href="#my-requests" className="portal-secondary">My Requests ↓</a>
    <h3 id="my-requests">My Requests</h3><div className="req-toolbar"><label className="req-field">Find a request<input type="search" placeholder="Title, counterparty or request number" value={query} onChange={e => setQuery(e.target.value)} /></label><label className="req-field">Status<select value={status} onChange={e => setStatus(e.target.value)}><option value="">Every status</option>{Object.entries(REQUEST_STATUSES).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>{options?.canManage && <label className="req-field">Show<select value={view} onChange={e => setView(e.target.value)}><option value="">{options.canSeeAll ? 'All client requests' : 'My workspace'}</option><option value="assigned">Assigned to me</option><option value="mine">Raised by me</option></select></label>}</div>
    ...
    <p className="req-help">Requestors see their own requests. Assigned lawyers and legal leads can review the linked term sheets and agreements.</p>
  </div>;
}
