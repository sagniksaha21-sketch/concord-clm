...
export default function AgreementSignatures({ agreementId }: { agreementId?: string }) {
  ...
  async function load() {
    ...
    setContracts(agreementId ? nextContracts.filter(c => c.id === agreementId) : nextContracts); setItems(agreementId ? nextItems.filter(r => r.contractId === agreementId) : nextItems); setArchive(agreementId ? nextArchive.filter(r => r.contractId === agreementId) : nextArchive);
    const requested = agreementId ?? new URLSearchParams(window.location.search).get('contract');
  ...
  useEffect(() => { load().catch((e) => setError(e instanceof Error ? e.message : 'E-signature workspace unavailable.')).finally(() => setLoading(false)); }, [agreementId]);
