...
export default function LifecycleRail({ stage, requestor = false, waiting = false, executed = false }: { stage: string; requestor?: boolean; waiting?: boolean; executed?: boolean }) {
  const steps = requestor ? [['intake','Submitted'],['assigned','Assigned'],['drafting','Drafting'],['review','Business input'],['negotiation','Negotiation'],['approval','Approval'],['signature','Signature'],['active','Completed']] : LIFECYCLE;
  const active = requestor && waiting ? 3 : requestor && stage === 'intake' ? 1 : steps.findIndex(s => s[0] === (stage === 'renewal' ? 'active' : stage === 'agreed' ? 'approval' : stage));
