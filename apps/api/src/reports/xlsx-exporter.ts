...
export function buildXlsx(report: PortfolioReport): Buffer {
  const overview: Cell[][] = [
    ...
    ['', ''],
    ['Insights', 'Finding', 'Tone'],
    ...report.insights.map((insight) => [insight.title, insight.body, insight.tone]),
    ['', ''],
    ['Stage', 'Agreements'],
  ...
  const entries = [
    ...
    { name: 'xl/worksheets/sheet1.xml', data: worksheet(overview, [34, 82, 28]) },
