...
export function buildXlsx(report: PortfolioReport): Buffer {
  const overview: Cell[][] = [
    ...
    ['Scope', report.selectionSummary ?? 'All agreements visible to the signed-in role'],
    ['Custom brief', report.options?.prompt ?? ''],
    ['Brief status', report.options?.prompt ? report.ai?.status === 'generated' ? 'AI-assisted narrative generated for this brief' : 'Not applied: AI unavailable; calculated findings retained' : 'Default portfolio narrative'],
    ['', ''],
