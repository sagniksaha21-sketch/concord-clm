...
export function appReturnPath(value: unknown): string | undefined {
  if (approvalReturnPath(value)) return value as string;
...
export function signedInHome(role: string, returnTo?: unknown): string {
  if (role === 'approver') return approvalReturnPath(returnTo) ?? '/approvals';
  return role === 'requester' ? requestReturnPath(returnTo) ?? '/requests' : appReturnPath(returnTo) ?? '/';
}
export function approvalReturnPath(value: unknown): string | undefined {
  return typeof value === 'string' && value.trim() === value && /^\/(?:inbox|approvals(?:\/[A-Za-z0-9-]+)?)$/.test(value) ? value : undefined;
}
