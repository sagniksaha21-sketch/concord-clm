...
export default function ExercisePicker(){
 ...
 return <SafeAreaView style={[styles.safe,{backgroundColor:theme.bg}]}><View style={styles.header}><ForgeBrand compact/><Pressable onPress={()=>router.back()}><Text style={[styles.close,{color:theme.muted}]}>CLOSE</Text></Pressable></View><ScrollView contentContainerStyle={styles.content}>
  ...
  <View style={{gap:8}}>{list.slice(0,50).map((e:any,i:number)=><Pressable key={e.id} onPress={()=>choose(e.id)}><GlassCard style={styles.row}><View style={{flex:1,minWidth:0}}><View style={styles.nameRow}><Text style={[styles.name,{color:theme.text}]}>{e.name}</Text>{params.mode==='workout'&&i<3?<Text style={[styles.best,{color:theme.amber,borderColor:theme.amber}]}>BEST MATCH</Text>:null}</View><Text style={[styles.meta,{color:theme.muted}]}>{e.group} · {String(e.movement).replaceAll('_',' ')} · {e.bodyweight?'bodyweight':`${e.increment||2.5} kg steps`}</Text></View><Text style={{fontSize:20,color:theme.gold}}>›</Text></GlassCard></Pressable>)}</View>
 </ScrollView></SafeAreaView>
}
const styles=StyleSheet.create({safe:{flex:1},header:{height:60,paddingHorizontal:16,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},close:{fontSize:8.5,fontWeight:'900',letterSpacing:1.2},content:{padding:16,paddingBottom:42,gap:13},kicker:{fontSize:8.5,fontWeight:'900',letterSpacing:1.5},h1:{fontSize:28,fontWeight:'1000',letterSpacing:-1},copy:{fontSize:10.5,lineHeight:16,fontWeight:'620'},search:{height:48,borderWidth:1,borderRadius:15,paddingHorizontal:13,fontWeight:'700'},summary:{flexDirection:'row',justifyContent:'space-between'},micro:{fontSize:8,fontWeight:'900',letterSpacing:.8},row:{flexDirection:'row',alignItems:'center',gap:10,padding:12},nameRow:{flexDirection:'row',gap:7,alignItems:'center'},name:{fontSize:12.5,fontWeight:'900',flexShrink:1},best:{fontSize:6.5,fontWeight:'1000',letterSpacing:.6,borderWidth:1,borderRadius:999,paddingHorizontal:6,paddingVertical:3},meta:{fontSize:8.5,fontWeight:'650',marginTop:4}});
