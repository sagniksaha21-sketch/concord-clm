...
import { SafeAreaView,View,Text,StyleSheet,Pressable,Image,Alert,ScrollView } from 'react-native';
...
type Achievement={label:string;headline:string;mark:string;sub:string};

export default function ShareStudio(){
 const router=useRouter();
 const {state,theme}=useForge();
 const shotRef=useRef<any>(null);
 const [photo,setPhoto]=useState<string|null>(()=>state.progressPhotos?.at(-1)?.uri||state.progressPhotos?.at(-1)?.data||null);
  ...
  <ScrollView contentContainerStyle={styles.content}>
   ...
   <View style={styles.previewWrap}>
    <ViewShot ref={shotRef} options={{format:'png',quality:1}} style={[styles.shot,{aspectRatio:ratio,backgroundColor:'#070707'}]}>
      ...
      <View style={styles.cardBrand}><Text style={styles.cardForge}>FORGE</Text><Text style={styles.cardTag}>ADAPT · EXECUTE · EVOLVE</Text></View>
...
const styles=StyleSheet.create({
 ...
 achievementRow:{flexDirection:'row',gap:7},ach:{flex:1,borderWidth:1,borderRadius:13,paddingVertical:10,paddingHorizontal:5,alignItems:'center'},achText:{fontSize:7.6,fontWeight:'900',letterSpacing:.45,textAlign:'center'},
 previewWrap:{alignItems:'center'},shot:{width:'82%',borderRadius:24,overflow:'hidden'},cardBrand:{position:'absolute',left:18,top:18},cardForge:{color:'#FFF6E8',fontSize:24,fontWeight:'1000',fontStyle:'italic',letterSpacing:-1.5},cardTag:{color:'#F5A623',fontSize:5.8,fontWeight:'900',letterSpacing:1.35,marginTop:1},
