...
export function Body3D({groups=[],selectable=false,onSelectionChange,height=276,autoRotate=true}:Props){
 ...
 const [ready,setReady]=useState(false),[viewName,setViewName]=useState('Front'),[internal,setInternal]=useState<BodyMuscleGroup[]>(()=>groups.filter((x):x is BodyMuscleGroup=>GROUPS.includes(x as BodyMuscleGroup)));
 const glRef=useRef<any>(null),resRef=useRef<any>(null),rafRef=useRef<number|undefined>(undefined),mountedRef=useRef(true),angleRef=useRef(0),targetRef=useRef<number|null>(null),lastRef=useRef(0),resumeRef=useRef(0),sizeRef=useRef({w:320,h:height}),selectedRef=useRef<Set<BodyMuscleGroup>>(new Set(internal)),hotspotsRef=useRef<any[]>([]),dragRef=useRef({x:0,y:0,lastX:0,moved:false});
