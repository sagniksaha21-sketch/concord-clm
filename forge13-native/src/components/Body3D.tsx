...
function shader(gl:any,type:number,src:string,label:string){const s=gl.createShader(type);if(!s)throw new Error(`${label} shader alloc`);gl.shaderSource(s,src);gl.compileShader(s);if(!gl.getShaderParameter(s,gl.COMPILE_STATUS))throw new Error(`${label} shader compile: ${gl.getShaderInfoLog(s)||'unknown'}`);return s;}
function program(gl:any,vs:string,fs:string){const p=gl.createProgram();if(!p)throw new Error('program alloc');gl.attachShader(p,shader(gl,gl.VERTEX_SHADER,vs,'vertex'));gl.attachShader(p,shader(gl,gl.FRAGMENT_SHADER,fs,'fragment'));gl.linkProgram(p);if(!gl.getProgramParameter(p,gl.LINK_STATUS))throw new Error(`program link: ${gl.getProgramInfoLog(p)||'unknown'}`);return p;}
...
export function Body3D({groups=[],selectable=false,onSelectionChange,height=276,autoRotate=true}:Props){
 ...
 const glRef=useRef<any>(null),resRef=useRef<any>(null),rafRef=useRef<number|undefined>(undefined),mountedRef=useRef(true),contextGenerationRef=useRef(0),angleRef=useRef(0),targetRef=useRef<number|null>(null),lastRef=useRef(0),viewRef=useRef('Front'),resumeRef=useRef(0),sizeRef=useRef({w:320,h:height}),selectedRef=useRef<Set<BodyMuscleGroup>>(new Set(internal)),hotspotsRef=useRef<any[]>([]),dragRef=useRef({x:0,y:0,lastX:0,moved:false});
 ...
 useEffect(()=>{mountedRef.current=true;return()=>{mountedRef.current=false;contextGenerationRef.current++;if(rafRef.current!=null)cancelAnimationFrame(rafRef.current);const gl=glRef.current,r=resRef.current;try{if(gl&&r){gl.deleteBuffer?.(r.buf);gl.deleteBuffer?.(r.ibo);gl.deleteProgram?.(r.prog);}}catch{}glRef.current=null;resRef.current=null;};},[]);
 ...
 const onContextCreate=(gl:any)=>{const generation=++contextGenerationRef.current;try{setFailure(null);const geom=geometry();if(!geom.vertexCount||!geom.triangleCount)throw new Error('mesh decode: empty geometry');const prog=program(gl,VS,FS),loc={aPos:gl.getAttribLocation(prog,'aPos'),aNormal:gl.getAttribLocation(prog,'aNormal'),uScale:gl.getUniformLocation(prog,'uScale'),uTranslate:gl.getUniformLocation(prog,'uTranslate'),uBodyY:gl.getUniformLocation(prog,'uBodyY'),uProj:gl.getUniformLocation(prog,'uProj'),uCamY:gl.getUniformLocation(prog,'uCamY'),uColor:gl.getUniformLocation(prog,'uColor'),uSelA:gl.getUniformLocation(prog,'uSelA'),uSelB:gl.getUniformLocation(prog,'uSelB')};if(loc.aPos<0||loc.aNormal<0)throw new Error('program interface: missing vertex attribute');for(const [name,value] of Object.entries(loc)){if(name.startsWith('u')&&value==null)throw new Error(`program interface: missing ${name}`);}const packed=new Float32Array((geom.p.length/3)*6);for(let i=0;i<geom.p.length/3;i++){packed[i*6]=geom.p[i*3];packed[i*6+1]=geom.p[i*3+1];packed[i*6+2]=geom.p[i*3+2];packed[i*6+3]=geom.n[i*3];packed[i*6+4]=geom.n[i*3+1];packed[i*6+5]=geom.n[i*3+2];}const buf=gl.createBuffer(),ibo=gl.createBuffer();gl.bindBuffer(gl.ARRAY_BUFFER,buf);gl.bufferData(gl.ARRAY_BUFFER,packed,gl.STATIC_DRAW);gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,ibo);gl.bufferData(gl.ELEMENT_ARRAY_BUFFER,geom.i,gl.STATIC_DRAW);assertGl(gl,'buffers');glRef.current=gl;resRef.current={prog,loc,buf,ibo,count:geom.i.length};draw();assertGl(gl,'first frame');if(!mountedRef.current||generation!==contextGenerationRef.current)return;setReady(true);requestAnimationFrame(()=>{if(mountedRef.current&&generation===contextGenerationRef.current&&rafRef.current==null)rafRef.current=requestAnimationFrame(animate);});}catch(e:any){const reason=String(e?.message||e||'unknown renderer error').slice(0,96);console.warn('FORGE native 3D init failed',e);setFailure(reason);setReady(false);}};
