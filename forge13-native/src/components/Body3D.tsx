...
export function Body3D({groups=[],selectable=false,onSelectionChange,height=276,autoRotate=true}:Props){
 ...
 const glRef=useRef<any>(null),resRef=useRef<any>(null),rafRef=useRef<number|undefined>(undefined),angleRef=useRef(0),targetRef=useRef<number|null>(null),lastRef=useRef(0),resumeRef=useRef(0),sizeRef=useRef({w:320,h:height}),selectedRef=useRef<Set<BodyMuscleGroup>>(new Set(internal)),hotspotsRef=useRef<any[]>([]),dragRef=useRef({x:0,y:0,lastX:0,moved:false});
 ...
 const draw=()=>{
  ...
  setViewName(nearestView(angleRef.current));
 };
