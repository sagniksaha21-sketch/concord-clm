...
type Context = {
  ...
  startWorkout: (id?:string) => void; adjustInput: (key:'weight'|'reps'|'rir', delta:number) => void; addSet: () => void;
...
export function ForgeProvider({children}:{children:React.ReactNode}) {
  ...
  const startWorkout=useCallback((id?:string)=>{setState(s=>{const p=mergedProgram(s,id),first=p.exercises[0],inc=exerciseMap[first]?.increment||2.5;return {...s,selectedProgramId:p.id,workout:{startedAt:Date.now(),programId:p.id,exerciseIndex:0,exerciseSets:{},exerciseOrder:[...p.exercises],input:{weight:inc<=1?10:20,reps:8,rir:2,setType:'working'},inputExerciseId:first,restTimerEndsAt:null,restSeconds:Number(s.settings.restSeconds||90)}};});Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium).catch(()=>{});},[]);
