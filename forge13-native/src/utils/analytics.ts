...
export function last30(state: ForgeState) {
  ...
  const sessions = (state.sessions || []).filter(s => { const t=Date.parse(s.date || ''); return Number.isFinite(t)&&t >= cutoff; });
  let sets = 0, volume = 0;
...
export function latestSession(state: ForgeState): SessionLog | undefined {
  return [...(state.sessions || [])].filter(s=>Number.isFinite(Date.parse(s.date||''))).sort((a,b)=>Date.parse(b.date)-Date.parse(a.date))[0];
}

export function weekSessions(state: ForgeState) {
  const cutoff = Date.now() - 7 * 864e5;
  return (state.sessions || []).filter(s => { const t=Date.parse(s.date || ''); return Number.isFinite(t)&&t >= cutoff; }).length;
}
