...
export function trainingRecommendation(state:ForgeState):TrainingRecommendation{
 const r=readiness(state);const recent=(state.sessions||[]).slice(0,4);const declining=recent.length>=3&&recent[0].volume<recent[1].volume&&recent[1].volume<recent[2].volume;
